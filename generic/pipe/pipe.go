...
func (p *pipe[T]) read() (v T, err error) {
	select {
	case <-p.done:
		return v, p.readCloseError()
	default:
	}
...
type PipeReader[T any] struct{ pipe[T] }
...
func (r *PipeReader[T]) Read() (v T, err error) {
	return r.pipe.read()
}
...
type PipeWriter[T any] struct{ r PipeReader[T] }
...
func (w *PipeWriter[T]) Write(v T) error {
	return w.r.pipe.write(v)
}
...
func Pipe[T any]() (*PipeReader[T], *PipeWriter[T]) {
	pw := &PipeWriter[T]{r: PipeReader[T]{pipe: pipe[T]{
		wrCh: make(chan T),
