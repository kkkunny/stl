...
func TestEither_Left(t *testing.T) {
	v := Left[int, int](1)
	stltest.AssertEq(t, pair.NewPair(v.Left()), pair.NewPair(1, true))
	v.SetLeft(2)
	stltest.AssertEq(t, pair.NewPair(v.Left()), pair.NewPair(2, true))
}

func TestEither_Right(t *testing.T) {
	v := Right[int, int](1)
	stltest.AssertEq(t, pair.NewPair(v.Right()), pair.NewPair(1, true))
	v.SetRight(2)
	stltest.AssertEq(t, pair.NewPair(v.Right()), pair.NewPair(2, true))
}
